  ...
  <div class="table-wrapper">
    <table class="table table-hover">
      ...
      <?php $i=1; foreach ($customers as $c):
        ...
        $caddr   = $c['address']    ?? '—';
        $ctype   = $c['type']       ?? 'retail';
        $cpts    = intval($c['points']     ?? 0);
        $ccreated= $c['created_at'] ?? date('Y-m-d H:i:s');
        $typeBadge = ['retail'=>['Lẻ','badge-secondary'],'wholesale'=>['Sỉ','badge-info'],'vip'=>['VIP','badge-warning']];
        [$tlabel, $tclass] = $typeBadge[$ctype] ?? ['Lẻ','badge-secondary'];
      ?>
        <tr>
          <td style="color:#aaa"><?= $i++ ?></td>
          <td>
            <div style="font-weight:600"><?= htmlspecialchars($cname) ?></div>
            <?php if (!empty($cemail)): ?><div style="font-size:11px;color:#aaa"><?= htmlspecialchars($cemail) ?></div><?php endif; ?>
          </td>
          <td><?= htmlspecialchars($cphone) ?></td>
          <td style="font-size:12px;color:#aaa"><?= htmlspecialchars($caddr) ?></td>
          <td><span class="badge <?= $tclass ?>"><?= $tlabel ?></span></td>
          <td style="text-align:center"><?= $orderCount[$cid] ?? 0 ?></td>
          <td style="font-weight:600;color:#E85D04"><?= vnd($orderTotal[$cid] ?? 0) ?></td>
          <td><span class="badge badge-success">⭐ <?= $cpts ?></span></td>
          <td style="color:#aaa;font-size:12px"><?= date('d/m/Y', strtotime($ccreated)) ?></td>
          <td>
            <form method="POST" style="display:inline">
              <input type="hidden" name="action" value="delete">
              <input type="hidden" name="id" value="<?= htmlspecialchars($cid) ?>">
              <button class="btn btn-sm btn-outline-danger" onclick="return confirmDelete('Xoá «<?= htmlspecialchars($cname) ?>»?')">
