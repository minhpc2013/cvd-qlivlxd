  ...
  <div class="table-wrapper">
    <table class="table table-hover">
      ...
      <?php foreach ($orders as $o):
        ...
        $paid    = floatval($o['paid']  ?? $total);
        $debt    = floatval($o['debt']  ?? max(0, $total - $paid));
        $status  = $o['status']        ?? 'pending';
        $created = $o['created_at']    ?? date('Y-m-d H:i:s');
      ?>
        <tr>
          <td style="font-weight:700;color:#E85D04">#<?= htmlspecialchars($oid) ?></td>
          <td style="font-weight:600"><?= htmlspecialchars($cname) ?></td>
          <td style="color:#aaa"><?= htmlspecialchars($phone) ?></td>
          <td style="font-size:12px;max-width:200px">
            <?php
              $itemLabels = array_map(
                fn($i) => htmlspecialchars(($i['name'] ?? '?') . ' x' . ($i['qty'] ?? 0)),
                array_slice($items, 0, 2)
              );
              echo implode(', ', $itemLabels);
            ?>
            <?php if (count($items) > 2): ?><em style="color:#aaa"> +<?= count($items)-2 ?> nữa</em><?php endif; ?>
          </td>
          <td style="font-weight:700"><?= vnd($total) ?></td>
          <td style="color:#10b981;font-weight:600"><?= vnd($paid) ?></td>
          <td style="color:<?= $debt > 0 ? '#ef4444' : '#10b981' ?>;font-weight:600">
            <?= vnd($debt) ?>
          </td>
          ...
          <td>
            <button class="btn btn-sm btn-outline-primary"
                    ...
                    onclick="setOrderId('<?= htmlspecialchars($oid) ?>','<?= htmlspecialchars($status) ?>')">
