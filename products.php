      ...
      <?php $i = 1; foreach ($products as $p): ?>
        <tr>
          ...
          <td>
            ...
            <?php if (!empty($p['note'])): ?>
              <div style="font-size:11px;color:#aaa"><?= htmlspecialchars($p['note']) ?></div>
            <?php endif; ?>
          </td>
          <td><span class="badge badge-info"><?= $p['category'] ?></span></td>
          <td><?= htmlspecialchars($p['unit']) ?></td>
          <td><?= vnd($p['price_buy'] ?? 0) ?></td>
          <td style="font-weight:700;color:#E85D04"><?= vnd($p['price_sell']) ?></td>
          <td style="color:#10b981;font-weight:600">
            <?php $profit = ($p['price_sell'] - ($p['price_buy']??0)); ?>
            +<?= vnd($profit) ?>
          </td>
          <td>
            <span class="badge badge-<?= $p['stock'] <= 3 ? 'danger' : ($p['stock'] <= 10 ? 'warning' : 'success') ?>">
              <?= $p['stock'] ?> <?= $p['unit'] ?>
            </span>
          </td>
          <td>
            ...
            <form method="POST" style="display:inline">
              ...
              <button class="btn btn-sm btn-outline-danger"
                      onclick="return confirmDelete('Xoá sản phẩm «<?= htmlspecialchars($p['name']) ?>»?')">
